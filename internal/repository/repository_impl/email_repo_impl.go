	...
	messagesInfo := []mailjet.InfoMessagesV31{
		{
			From: &mailjet.RecipientV31{
				Email: global.Config.MailJet.From_mail,
				Name: global.Config.MailJet.From_name,
			},
...
func (e *EmailRepositoryImpl) checkMailjetConfig() error {
	...
	if global.Config.MailJet.API_key == "" || global.Config.MailJet.Secret_key == "" || global.Config.MailJet.From_mail == "" {
		e.logger.LogWarning(
			"Cấu hình Mailjet chưa đầy đủ. Vui lòng kiểm tra lại file cấu hình.", "",
			zap.Bool("api_key_set:", global.Config.MailJet.API_key != ""),
			zap.Bool("secret_key_set:", global.Config.MailJet.Secret_key != ""),
		)
		e.logger.LogInfo("Email có thể thiếu đi thông tin xác thực","",
			zap.String("api_key", global.Config.MailJet.API_key),
			zap.String("secret_key", global.Config.MailJet.Secret_key),
			zap.String("from_mail", global.Config.MailJet.From_mail),
		)
	}
	return nil
}